      ...
      <div className="min-h-screen">
        ...
        <main className="section-padding">
          <div className="container-custom text-center">
            ...
            <Link to="/projects" className="btn-primary inline-flex gap-2"><RotateCcw size={16} /> Back to Projects</Link>
          </div>
        </main>
        <Footer />
      </div>
        ...
        <div className="container-custom">
          <div className="text-sm breadcrumbs mb-6 text-gray-500">
            <ul>
              ...
              <li><Link to="/projects" className="hover:text-timbercraft-green">Projects</Link></li>
              <li className="text-timbercraft-dark font-medium">{project.title}</li>
            </ul>
          </div>

          <span className="badge badge-lg bg-timbercraft-green text-white border-none font-semibold mb-3">{project.category}</span>
          <h1 className="text-3xl md:text-4xl font-display font-semibold text-timbercraft-dark mb-2 tracking-tight">{project.title}</h1>
          ...
          {project.gallery && project.gallery.length > 1 && (
            <div className="grid grid-cols-4 gap-4">
              {project.gallery.map((img, idx) => (
                ...
                >
                  <img src={img} alt={`${project.title} ${idx + 1}`} className="w-full h-24 md:h-28 object-cover" />
                </button>
              ))}
            </div>
          )}
          ...
          <div className="mt-12 text-center">
            <Link to="/contact" className="btn-primary btn-lg inline-flex">Start Your Project →</Link>
          </div>
        </div>
      </main>
      <Footer />
      ...
      {lightboxOpen && (
        <div className="modal modal-open" onClick={() => setLightboxOpen(false)}>
          <div className="modal-box max-w-5xl p-2 bg-transparent shadow-none">
            <img src={activeImage} alt={project.title} className="w-full h-auto rounded-lg" />
          </div>
